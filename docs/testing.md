...
**Day 16 complete (scaffold only).** IAM least-privilege upload user is defined in `infra/aws/iam.tf`. Run `terraform plan` to preview; do **not** `apply` until Day 20. Access keys go in local `.env` only after apply. No upload script yet (`make upload-gold-s3` is Day 19). Snowflake is Week 4+.
